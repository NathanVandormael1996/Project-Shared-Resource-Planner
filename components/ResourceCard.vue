...
<template>
  <div class="bg-[#24262d] rounded-xl border border-gray-700 hover:border-blue-500 transition flex justify-between items-center group overflow-hidden">
    ...
    <div class="flex-1 px-5 py-2 min-w-0">
      ...
      <div class="flex gap-4 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button @click.stop="emit('edit', resource)" class="text-xs font-bold text-amber-500 hover:underline">BEWERKEN</button>
        <button @click.stop="emit('delete', resource.id)" class="text-xs font-bold text-red-500 hover:underline">VERWIJDEREN</button>
      </div>
    </div>
    ...
    <div class="pr-5 flex flex-col items-end gap-2">
      ...
      <button
          @click.stop="emit('reserve', resource)"
